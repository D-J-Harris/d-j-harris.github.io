...
deep learning; explaining what it is, some key terminology in the field, and focusing on communication between machines
in this setting. There are various methods of approaching this, and I will be looking most closely at 'gradient
compression', which describes methods by which machines are able talk to each other in more memory-efficient ways, when
training large machine learning models.

## Take a Step Back, so Deep Learning?
...
+ *Synchronous SGD*: these 'gradient descent' steps can be simply extended to a multi-worker setting. If batches of data
...
+ *Asynchronous SGD*: Workers could instead send their step updates as and when they have them, eliminating this
...
### Parallelism
+ *Data Parallelism*: In this setting, the data that we want to train on is split into non-overlapping batches, and 
...
+ *Model Parallelism*: In the case where a model is too big to fit in memory on a single device, it is useful to rather
...
## Sounds Great, But How Costly is Communicating Updates?

You didn't ask, but I did. Because the answer is: pretty costly. For example, the model size of ResNet-50 is over
...
<div class="caption">
<b>Figure 1.</b> Sketch demonstrating the effect of gradient quantisation and sparsification on a single gradient
vector. Quantisation reduces bit-representation across all elements, whereas sparsification methodically selects only a
few full elements. Both techniques reduce gradient size.
(Image source: <a href="https://arxiv.org/pdf/2003.06307.pdf">Tang et al.</a>)
</div>
<br>
...
I'm only scratching the surface here! This whistle-stop tour of distributed deep learning reflects on the richness in
this area for future study. In my dissertation I plan to more closely review these gradient compression techniques,
along with a host of more recent exciting advances, from Sketch communication
...
I hope to revisit this topic on my site in a month or so after exams, most likely with a Monthly Mouthful segment looking 
at deeper dives into the theory and implementation of some of the more juicy compression algorithms. Until then, I hope
the above has whetted the appetite of all zero of my readers!
